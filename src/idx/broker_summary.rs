//! IDX daily broker summary (per-broker buy/sell) — `GetBrokerSummary`.
...
/// Fetch the broker summary for `date` (YYYY-MM-DD; latest trading day if `None`).
///
/// # Errors
///
...
pub async fn fetch(client: &IdxClient, date: Option<&str>) -> Result<Value> {
    ...
    client.trading_summary("GetBrokerSummary", date).await
}
